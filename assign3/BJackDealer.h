...
public:
  ...
  /**
   * retain the whole set of cards and shuffle , use a random number be a new seed
   */
  void shuffle(void);
  ...
  /**
   * clear the cards dealer have currently
   */
  void clearCards();
...
private:
  ...
  static const int kMaxCards=10;
  int cardnum;	// quantity of cards
  int decknum;	// quantity of deck
  int cardstatus[kMaxNCards];	// the remains cards 0 ~ 51
  BJackPlayer *dealer;	// dealer is also a player in the game
};
